...
def get_clips(video_path, video_begin, video_end, label, view, sample_duration):
    ...
    seven = int(sample_duration / 2) - step
    nine = int(sample_duration / 2) + step
    eight = int(sample_duration / 2)

    if video_begin == 0:
        for i in range(seven):
            if len([0] * (seven - i) + list(range(0, i + nine))) == sample_duration:
                sample_ = sample.copy()
                sample_['frame_indices'] = [0] * (seven - i) + list(range(0, i + nine))
                clips.append(sample_)
        for i in range(seven, video_end + 1, step):
            if len(list(range(i - seven, i + nine))) == sample_duration:
                sample_ = sample.copy()
                sample_['frame_indices'] = list(range(i - seven, i + nine))
                clips.append(sample_)

    elif video_end == 9999:
        for i in range(video_begin, 9999 - seven, step):
            if i + nine < 9999 and len(list(range(i - seven, i + nine))) == sample_duration:
                sample_ = sample.copy()
                sample_['frame_indices'] = list(range(i - seven, i + nine))
                clips.append(sample_)
        for i in range(eight):
            if len(list(range(9999 - seven * 2 + i, 10000)) + [9999] * (i + 1)) == sample_duration:
                sample_ = sample.copy()
                sample_['frame_indices'] = list(range(9999 - seven * 2 + i, 10000)) + [9999] * (i + 1)
                clips.append(sample_)
    else:
