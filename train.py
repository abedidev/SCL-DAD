...
from testdad import evaluating, get_normal_vector_head, cal_score_head
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        anormal_data = DAD(root_path=args.root_path,
                           subset='train',
                           view=args.view,
        ...
        training_anormal_size = int(len(anormal_data) * args.a_split_ratio * .8)
        training_anormal_data = torch.utils.data.Subset(anormal_data, np.arange(training_anormal_size))
        ...
        normal_data = DAD(root_path=args.root_path,
                          subset='train',
                          view=args.view,
                          sample_duration=before_crop_duration,
                          type='normal',
        ...
        training_normal_size = int(len(normal_data) * args.n_split_ratio * .8)
        training_normal_data = torch.utils.data.Subset(normal_data, np.arange(training_normal_size))
        ...
        print(
            "=================================Loading Anormal-Driving Validation Data!=================================")
        ...
        validation_anormal_size = int(len(anormal_data) * args.a_split_ratio * .2)
        validation_anormal_data = torch.utils.data.Subset(anormal_data, np.arange(training_anormal_size, int(len(
            anormal_data) * args.a_split_ratio)))

        validation_anormal_loader = torch.utils.data.DataLoader(
            validation_anormal_data,
        ...
        print(
            "=================================Loading Normal-Driving Validation Data!=================================")
        validation_normal_size = int(len(normal_data) * args.n_split_ratio * .2)
        validation_normal_data = torch.utils.data.Subset(normal_data, np.arange(training_normal_size, int(len(
            normal_data) * args.n_split_ratio)))

        validation_normal_loader = torch.utils.data.DataLoader(
            validation_normal_data,
            batch_size=args.val_batch_size,
            shuffle=True,
            num_workers=args.n_threads,
            pin_memory=True,
        ...
        print(f'len_neg: {validation_anormal_data.__len__()}')
        print(f'len_pos: {validation_normal_data.__len__()}')
        ...
        if args.resume_path == '':
            ...
            best_auc = resume_checkpoint['auc']
        ...
        val_logger = Logger(os.path.join(args.log_folder, 'val.log'),
                            ['epoch', 'auc', 'prauc', 'normal_acc'], args.log_resume)
        ...
        for epoch in range(begin_epoch, begin_epoch + args.epochs + 1):
            ...
            if epoch % args.val_step == 0:
                ...
                model.eval()
                auc, prauc = evaluating(model, normal_vec, validation_anormal_loader, validation_normal_loader,
                                        args.use_cuda)
                print(
                    f'Epoch: {epoch}/{args.epochs} | AUC: {auc} | AUC of PR: {prauc}')
                print(
                    "==========================================!!!Logging!!!==========================================")
                val_logger.log({
                    'epoch': epoch,
        ...
        model_front_ir_head = resnet.ProjectionHead(args.feature_dim, args.model_depth)
        model_top_d_head = resnet.ProjectionHead(args.feature_dim, args.model_depth)
        model_top_ir_head = resnet.ProjectionHead(args.feature_dim, args.model_depth)

        resume_path_front_d_head = './checkpoints/best_model_' + args.model_type + '_front_depth_head.pth'
        resume_path_front_ir_head = './checkpoints/best_model_' + args.model_type + '_front_IR_head.pth'
        resume_path_top_d_head = './checkpoints/best_model_' + args.model_type + '_top_depth_head.pth'
        resume_path_top_ir_head = './checkpoints/best_model_' + args.model_type + '_top_IR_head.pth'

        resume_checkpoint_front_d_head = torch.load(resume_path_front_d_head)
        resume_checkpoint_front_ir_head = torch.load(resume_path_front_ir_head)
        resume_checkpoint_top_d_head = torch.load(resume_path_top_d_head)
        resume_checkpoint_top_ir_head = torch.load(resume_path_top_ir_head)

        model_front_d_head.load_state_dict(resume_checkpoint_front_d_head['state_dict'])
        model_front_ir_head.load_state_dict(resume_checkpoint_front_ir_head['state_dict'])
        model_top_d_head.load_state_dict(resume_checkpoint_top_d_head['state_dict'])
        model_top_ir_head.load_state_dict(resume_checkpoint_top_ir_head['state_dict'])

        model_front_d_head.cuda()
        model_front_ir_head.cuda()
        model_top_d_head.cuda()
        model_top_ir_head.cuda()

        model_front_d_head.eval()
        model_front_ir_head.eval()
        model_top_d_head.eval()
        model_top_ir_head.eval()
        ...
        normal_vec_front_ir = get_normal_vector_head(model_front_ir,
                                                     model_front_ir_head,
        ...
        normal_vec_top_d = get_normal_vector_head(model_top_d,
                                                  model_top_d_head,
                                                  train_normal_loader_for_test_top_d,
                                                  args.cal_vec_batch_size,
                                                  args.feature_dim,
                                                  args.use_cuda)
        np.save(os.path.join(args.normvec_folder, 'normal_vec_top_d.npy'), normal_vec_top_d.cpu().numpy())

        normal_vec_top_ir = get_normal_vector_head(model_top_ir,
                                                   model_top_ir_head,
                                                   train_normal_loader_for_test_top_ir,
                                                   args.cal_vec_batch_size,
                                                   args.feature_dim,
                                                   args.use_cuda)
        np.save(os.path.join(args.normvec_folder, 'normal_vec_top_ir.npy'), normal_vec_top_ir.cpu().numpy())

        cal_score_head(model_front_d, model_front_ir, model_top_d, model_top_ir,
        ...
        np.save(os.path.join(args.score_folder, 'gt.npy'), gt)
        ...
        for mode, mode_name in hashmap.items():
            ...
            best_auc, best_threshold, AUC = evaluate(score, gt, False)
            print(
                f'Mode: {mode_name}:      Best Acc: {round(best_auc, 2)} | Threshold: {round(best_threshold, 2)} | AUC: {round(AUC, 4)}')
            score = post_process(score, args.window_size)
            best_auc, best_threshold, AUC = evaluate(score, gt, False)
            print(
                f'View: {mode_name}(post-processed):       Best Acc: {round(best_auc, 2)} | Threshold: {round(best_threshold, 2)} | AUC: {round(AUC, 4)} \n')
